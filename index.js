...
let blog = mysql.createConnection({
  host: "localhost",
  user: "root",
  database: "blog",
  password: "viren021004",
});

//home page
app.get("/", (req, res) => {
  let q = "SELECT * FROM USER ORDER BY date DESC";
  try {
...
app.post("/add-user", (req, res) => {
  ...
  let q = `INSERT INTO USER (id, name, email, para, password, enroll, date) VALUES (?, ?, ?, ?, ?, ?, CURDATE())`;
  try {
...
app.post("/delete/:id", (req, res) => {
  ...
  const qSelect = `SELECT password FROM USER WHERE id='${id}'`;
...
app.post("/update/:id", (req, res) => {
  ...
  const authQuery = `SELECT PASSWORD FROM user WHERE id = ?`;
  ...
  blog.query(authQuery, [id], (err, result) => {
    ...
    if (result.length > 0 && result[0].PASSWORD === password) {
      const updateQuery = `UPDATE user SET name = ?, para = ?, email = ?,enroll=?,date = IFNULL(?, CURDATE()) WHERE id = ?`;
