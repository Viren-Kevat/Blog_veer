...
app.post("/update/:id", (req, res) => {
  ...
  const authQuery = `SELECT PASSWORD FROM railway.users WHERE id = ?`;
  ...
  blog.query(authQuery, [id], (err, result) => {
    ...
    if (result.length > 0 && result[0].PASSWORD === password) {
      const updateQuery = `UPDATE railway.users SET name = ?, para = ?, email = ?,enroll=?,date = IFNULL(?, CURDATE()) WHERE id = ?`;
