...
//home page
app.get("/", (req, res) => {
  let q = "SELECT * FROM railway.viren ORDER BY date DESC";
  try {
...
app.post("/add-user", (req, res) => {
  ...
  let q = `INSERT INTO railway.viren (id, name, email, para, password, enroll, date) VALUES (?, ?, ?, ?, ?, ?, CURDATE())`;
  try {
...
app.post("/delete/:id", (req, res) => {
  ...
  const qSelect = `SELECT password FROM railway.viren WHERE id='${id}'`;
  blog.query(qSelect, (err, results) => {
    ...
    if (results.length > 0 && results[0].password === userPassword) {
      const qDelete = `DELETE FROM railway.viren WHERE id='${id}'`;
...
app.get("/user/:id", (req, res) => {
  let { id } = req.params;
  let q = `SELECT * FROM railway.viren WHERE id='${id}'`;
  try {
    blog.query(q, (err, result) => {
      if (err) throw err;
      let post = result[0];
...
app.post("/update/:id", (req, res) => {
  ...
  const authQuery = `SELECT PASSWORD FROM railway.viren WHERE id = ?`;
  ...
  blog.query(authQuery, [id], (err, result) => {
    ...
    if (result.length > 0 && result[0].PASSWORD === password) {
      const updateQuery = `UPDATE railway.viren SET name = ?, para = ?, email = ?,enroll=?,date = IFNULL(?, CURDATE()) WHERE id = ?`;
